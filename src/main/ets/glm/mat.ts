...
export interface tmat<T extends valuetype, R extends Rows, C extends Cols> {
    elements: T;
    rows: R;
    cols: C;
...
export class BaseMat<T extends valuetype, R extends Rows, C extends Cols> implements tmat<T, R, C> {
    readonly elements: T;
    readonly rows: R;
    readonly cols: C;
    ...
    static createNew<T extends valuetype, R extends Rows, C extends Cols>(m: mat<T, R, C>, data: T): mat<T, R, C> {
        let t = m as BaseMat<T, R, C>;
        return new BaseMat(t.rows, t.cols, t.elements.constructor as TypeConstructor<T>, data);
    }

    clone(): mat<T, R, C> {
        return new BaseMat(this.rows, this.cols, this.elements.constructor as TypeConstructor<T>, (this.elements.constructor as TypeConstructor<T>).from(this.elements));
    }
    ...
    get transpose(): mat<T, C, R> {
        const result = new BaseMat(this.cols, this.rows, this.elements.constructor as TypeConstructor<T>);
    ...
    get adjugate(): mat<T, R, C> {
        return new BaseMat(this.rows, this.cols, this.elements.constructor as TypeConstructor<T>, this.adjugate2());
    }
    ...
    get inverse(): mat<T, R, C> {
        ...
        return new BaseMat(this.rows, this.cols, this.elements.constructor as TypeConstructor<T>, inverseMatrix);
    }
    ...
    mul<R1 extends Rows>(m: mat<T, R1, R>): mat<T, R1, C> {
        ...
        const result = new BaseMat(m.rows, this.cols, this.elements.constructor as TypeConstructor<T>);
